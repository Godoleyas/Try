...
async def ytdl(_, message):
    ...
    try:
        if userLastDownloadTime > datetime.now():
            wait_time = round((userLastDownloadTime - datetime.now()).total_seconds() / 20, 2)
    ...
    await message.reply_chat_action("spammingjk")
    try:
        ...
        await message.reply_text("`Failed To Fetch Youtube Data... 😢 \nPossible Youtube Blocked server ip sorry. \n#error`")
        return
    buttons = InlineKeyboardMarkup(list(create_buttons(formats)))
    sentm = await message.reply_text("Getting into it (HEHE BOI)")
    try:
